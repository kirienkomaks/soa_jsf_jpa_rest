...
public class LibraryPersistentBean implements LibraryPersistentBeanLocal {
    ...
    public void addBook(Book book, Category category, Author author) {
        try {
            ...
            queueSender.sendMessage("Ksiązka " + book.getTitle() + " dodana");
            addCatalog(book);
    ...
    @Lock
    @Transactional
    public void addObservations(User user, Book book){
        UserObservation userObservation = new UserObservation();
        userObservation.setUser(user);
        userObservation.setIdBook(book.getIdBook());
        em.getTransaction().begin();
    ...
    public void sendToUsers(){
        ...
        //String message = queueReceiver.startReceiver();
        String message = mdb.getMessage();
        for (User u:users) {
            if (u.isNotifications()){
                insertMessage(u,message);
    ...
    @Lock
    @Transactional
    public void sendToObservations(Book book){
        List<User> users = getUsers();

        //String message = queueReceiver.startReceiver();
        String message = mdb.getMessage();
        System.out.println( mdb.getMessage());
        for (User u:users) {
            for (UserObservation uo:u.getUserObservations()) {
                if (uo.getIdBook()==book.getIdBook()){
                    insertMessage(u,message);
                    System.out.println(message);
                }
            }
    ...
    @Lock
    @Transactional
    public void addUser(String name, String surname, Boolean notifications) {
        em.getTransaction().begin();
        em.persist(new User(name, surname, notifications));
        em.getTransaction().commit();
    }
    ...
    public User getUser(String name, String surname) {
        ...
        try {
            ...
            user = new User(name, surname, true);
    ...
    public void returnBook(Book book) {
        try {
            ...
            List<Lending> lendings  = getLendingBooks();
            ...
            queueSender.sendMessage("Ksiązka " + book.getTitle() + " dostępna");
